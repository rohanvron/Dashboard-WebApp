...
const generateData = () => {
  return Array.from({ length: 100 }, () => {
    ...
    return {
      ...
      email: `${fullName.toLowerCase().replace(/\s/g, "")}${Math.floor(
        Math.random() * 90 + 10
      )}@${
        ["gmail.com", "yahoo.com", "hotmail.com", "google.com"][
...
const PeopleTable = () => {
  ...
  const confirmDelete = () => {
    if (deletingMember) {
      setData(data.filter((item) => item.id !== deletingMember.id));
  ...
  const handleAddMember = (newMember) => {
    setData([
      ...data,
      {
        ...
        username: `@${newMember.name.split(" ")[0].toLowerCase()}`,
      },
    ]);
  ...
  const handleEditMember = (editedMember) => {
    setData(
      data.map((item) =>
        item.id === editedMember.id ? { ...item, ...editedMember } : item
      )
    );
  ...
  const handleApplyFilters = (filters) => {
    setData(
      generateData().filter((item) => {
        return (
          (!filters.roles.length || filters.roles.includes(item.role)) &&
          (!filters.teams.length ||
            item.teams.some((team) => filters.teams.includes(team)))
        );
      })
    );
    setShowFilter(false);
